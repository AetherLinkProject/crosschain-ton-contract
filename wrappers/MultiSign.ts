...
export function MultiSignConfigToCell(config: MultiSignConfig): Cell {
    return beginCell()
        .storeUint(0, 256)
...
export class MultiSign implements Contract {
    public orderSeqno: number;

    constructor(readonly address: Address,
        readonly init?: { code: Cell; data: Cell },
        readonly configuration?: MultiSignConfig) {
        this.orderSeqno = 0;
    }

    static createFromAddress(address: Address) {
        return new MultiSign(address);
    }

    static createFromConfig(config: MultiSignConfig, code: Cell, workchain = 0) {
        const data = MultiSignConfigToCell(config);
        const init = { code, data };
        return new MultiSign(contractAddress(workchain, init), init, config);
    }
    ...
    async sendNewOrder(
        ...
        await provider.internal(via, {
            sendMode: SendMode.PAY_GAS_SEPARATELY,
            value,
            body: MultiSign.newOrderMessage(newActions, expirationDate, isSigner, addrIdx)
        });
